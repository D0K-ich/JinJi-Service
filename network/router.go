...
func CreateRouter(files_path string, admin_token string, user_session *session.Session) (main_router *router.Router) {
	...
	main_router.POST("/user/create/new", func(ctx *fasthttp.RequestCtx) {
		var err error
		...
		if check_cookie != nil {
			if user_id, err = strconv.Atoi(fmt.Sprintf("%v", session_store.Get(cookieKeyUser))); err != nil {
				return
			} //todo
		}
		...
		if response = handler.Route("user/new"); response == nil {
			writeError(ctx, "nil response generated by server")
			return
		}
